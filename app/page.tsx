            ...
            <Card className="mt-6">
              ...
              <div className="overflow-x-auto px-4 pb-4">
                <table className="w-full text-sm">
                  ...
                  <tbody>{filtered.map(t=><tr key={t.id} className="border-t hover:bg-zinc-50 group"><td className="py-3 text-xs">{formatDateView((t as any).data_vencimento || t.data)}</td><td className="py-3"><div className="font-medium text-xs">{t.descricao}</div>{(t.itens || t.impostos || t.observacao || (t.rateio_filiais && (typeof t.rateio_filiais === 'string' ? JSON.parse(t.rateio_filiais) : t.rateio_filiais).length > 0)) && (<div className="text-[10px] text-zinc-500 mt-1 flex flex-wrap gap-x-3 gap-y-1">{t.itens && <span>📦 {t.itens}</span>}{t.impostos && <span>🏛 Impostos: {t.impostos}</span>}{t.observacao && <span>📝 {t.observacao}</span>}{t.rateio_filiais && (typeof t.rateio_filiais === 'string' ? JSON.parse(t.rateio_filiais) : t.rateio_filiais).length > 0 && <span title={(typeof t.rateio_filiais === 'string' ? JSON.parse(t.rateio_filiais) : t.rateio_filiais).map((r:any) => `${r.empresa}: ${BRL.format(Number(r.valor))}`).join(' | ')} className="bg-violet-100 text-violet-700 px-2 rounded-full font-bold cursor-help">🏢 Rateado ({(typeof t.rateio_filiais === 'string' ? JSON.parse(t.rateio_filiais) : t.rateio_filiais).length} lojas)</span>}</div>)}</td><td className="py-3"><span className={`px-2 py-1 rounded-full text-[10px] ${(t as any).status==='a_pagar' ? 'bg-amber-100 text-amber-700' : (t as any).status==='a_receber' ? 'bg-emerald-100 text-emerald-700' : 'bg-zinc-100'}`}>{(t as any).status||'realizado'}</span></td><td className="py-3 text-right font-bold text-xs">{BRL.format(Number(t.valor))}</td><td className="py-3 text-right text-zinc-400 flex justify-end gap-3 opacity-10 md:opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={()=>toggleStatus(t)} className="hover:text-emerald-600" title="Conciliar / Desconciliar">{['pago','realizado'].includes((t as any).status||'realizado') ? <Undo2 className="w-4 h-4"/> : <Check className="w-4 h-4"/>}</button><button onClick={()=>editar(t)} className="hover:text-violet-600" title="Editar"><Pencil className="w-4 h-4"/></button><button onClick={()=>excluir(t.id)} className="hover:text-red-600" title="Excluir"><Trash2 className="w-4 h-4"/></button></td></tr>)}</tbody>
                </table>
              </div>
            </Card>
        )}
